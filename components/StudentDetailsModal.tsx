...
const StudentDetailsView: React.FC<StudentDetailsViewProps> = ({ student, plans, trainer, workoutTemplates, groups, onBack, onUpdate, onDelete, onAddPayment, allStudents }) => {
  ...
  const TabButton = ({ tab, label, Icon }: { tab: Tab, label: string, Icon: React.FC<{className?: string}> }) => (
    <button
        ...
        className={`flex-shrink-0 flex items-center gap-2 px-3 py-2 text-sm font-medium border-b-2 ${activeTab === tab ? 'border-brand-primary text-brand-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
    >
