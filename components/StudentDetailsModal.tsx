...
import { Student, Plan, ClassSession, Payment, PaymentMethod, ClassSessionType, Workout, StudentFile, ProgressPhoto, Schedule } from '../types';
...
const DetailsTab: React.FC<any> = ({ student, plans, isEditing, setIsEditing, editableStudent, setEditableStudent, handleSave, handleDelete, setPictureModalOpen, setPaymentModalOpen, handleAddSession, handleDeleteSession, onUpdate }) => {
    ...
    const handleDayChange = (dayId: string) => {
        const currentDays = editableStudent.schedule?.days || [];
                ...
                <div>
                    ...
                    <div className="mt-2 p-3 border rounded-md space-y-3 bg-gray-50">
                                ...
                                {daysOfWeek.map(day => (
                                    <button
                                        ...
                                        className={`px-3 py-1 text-sm rounded-full border ${
                                            editableStudent.schedule?.days?.includes(day.id)
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label htmlFor="startTime" className="text-xs font-medium text-gray-600">Início</label>
                                <input
                                    type="time"
                                    id="startTime"
                                    name="startTime"
                                    value={editableStudent.schedule?.startTime || ''}
                            ...
                            <div>
                                <label htmlFor="endTime" className="text-xs font-medium text-gray-600">Fim</label>
                                <input
                                    type="time"
                                    id="endTime"
                                    name="endTime"
                                    value={editableStudent.schedule?.endTime || ''}
                                    onChange={handleTimeChange}
                                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1 px-2 focus:outline-none focus:ring-brand-accent focus:border-brand-accent sm:text-sm"
                                />
                            </div>
